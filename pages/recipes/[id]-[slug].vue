<template>
    <div v-if="data == null">null</div>
    <div class="lg:flex lg:items-start lg:justify-between" v-if="data != null">
        <div class="min-w-0 flex-1">
            <h2 class="text-2xl font-bold leading-7 text-gray-700 sm:truncate sm:text-3xl sm:tracking-tight"> {{ data.name }} </h2>
        ...
        <div class="relative h-40 md:h-80 w-full overflow-hidden rounded-t-xl">
            ...
            <img class="absolute inset-0 w-full h-full object-cover"
                ...
                <div class="absolute bottom-0 left-1/2 -translate-x-1/2 bg-orange-50 flex gap-8 justify-center text-center px-4 @[300px]/grid:px-12 py-2 rounded-t-md">
                    <div>
                        <p class="font-medium text-orange-400">{{ Math.round(data.energy) || 0 }}</p>
                        <p class="text-xs text-orange-950/50">kcal</p>
                    </div>
                    <div>
                        <p class="font-medium text-orange-400">{{ Math.round(data.protein) || 0 }}</p>
                        <p class="text-xs text-orange-950/50">protein</p>
                    </div>
                    <div>
                        <p class="font-medium text-orange-400">{{ Math.round(data.fat) || 0 }}</p>
                        <p class="text-xs text-orange-950/50">fat</p>
                    </div>
                    <div>
                        <p class="font-medium text-orange-400">{{ Math.round(data.carbs) || 0 }}</p>
                        <p class="text-xs text-orange-950/50">carbs</p>
                    </div>
                    <div>
                        <p class="font-medium text-orange-400">{{ Math.round(data.fibres) || 0 }}</p>
